...
df <- read.table('~/Desktop/R/week_2/practice_2/test_data.csv', sep = ",", header = T, stringsAsFactors = F)
...
### Part III - 資料框整理技巧II #####################################################
df.sport <- data.frame(Name = c("Ali", "Petty", "Kobe"), FavSport = c("basketball", "baseball", "basketball"))
...
df$Bmi <- mapply(GetBmi, df$Height, df$Weight, MoreArgs = NULL, SIMPLIFY = T, USE.NAMES = T)
